...
function getNewMirrorCleanAndUpgrade() {
    ...
    curl -s "https://www.archlinux.org/mirrorlist/?country=RU&country=DE&protocol=https&use_mirror_status=on" | sed -e 's/^#Server/Server/' -e '/^#/d' | rankmirrors -n 3 - > /etc/pacman.d/mirrorlist
    pacman -Sc --noconfirm
